...
class _AnisotropicLayer:
    ...
    def _CalcEpsilonMatrix(self) -> None:
        ...
        self.epsTensor = np.dot(
            RotationSx(self.xi),
            np.dot(
                RotationSz(self.psi), np.dot(self.epsTensorCrystal, np.dot(RotationSz(-self.psi), RotationSx(-self.xi)))
            ),
        )
    ...
    def _SolveEigenFunction(self, beta: float) -> None:
        ...
        for i in range(4):
            poyntingX[i] = 0.5 * (vectors[0, i] * np.conj(vectors[1, i]) - vectors[2, i] * np.conj(vectors[3, i])).real
...
class TmmPy:
    ...
    def Solve(
        ...
        pBackward = np.array(
            [
                self.layers[0].poynting[1],
                self.layers[0].poynting[3],
                self.layers[-1].poynting[1],
                self.layers[-1].poynting[3],
            ]
        )

        pForward = np.array(
            [
                self.layers[0].poynting[0],
                self.layers[0].poynting[2],
                self.layers[-1].poynting[0],
                self.layers[-1].poynting[2],
            ]
        )
